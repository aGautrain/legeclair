              ...
              <div v-if="activeTab === 'comparison'" class="comparison-view">
                <div class="d-flex align-center justify-space-between mb-4">
                  <h3 class="text-h6 font-weight-semibold">
                    {{ $t("content_comparison") }}
                  </h3>
                  <div class="d-flex gap-2">
                    <v-btn
                      :prepend-icon="
                        showHighlighting ? 'mdi-eye-off' : 'mdi-eye'
                      "
                      size="small"
                      variant="outlined"
                  ...
                  <v-col cols="12" md="6">
                    <v-card class="content-card" variant="outlined">
                      ...
                      >
                        ...
                        <v-tooltip v-model="copiedOriginal" :text="$t('copy')">
                  ...
                  <v-col cols="12" md="6">
                    <v-card class="content-card" variant="outlined">
                      ...
                      >
                        ...
                        <v-tooltip v-model="copiedCorrected" :text="$t('copy')">
                          <template #activator="{ props }">
                            <v-btn
                              v-bind="props"
                      ...
                      <v-card-text class="content-text pa-4">
                        ...
                        >
                          <template
                            v-for="(segment, idx) in correctedSegments"
                            :key="idx"
                          >
                            <template v-if="segment.correction">
                              <v-tooltip
                                location="bottom"
                                :text="segment.correction.explanation"
                              >
                                <template #activator="{ props }">
                                  <span
                                    v-bind="props"
              ...
              >
                ...
                <v-list class="corrections-list">
                  ...
                  >
                    ...
                    <v-list-item-title
                      class="text-body-1 font-weight-medium py-2"
                    >
                        ...
                        <div class="d-flex gap-2 align-center">
                          <span>
                            <v-chip size="small" variant="outlined">
                              <v-icon class="mr-1" size="16">mdi-tag</v-icon>
...
const correctedSegments = computed(() => {
  ...
  if (corrections.length === 0) return [{ text: content }];
  ...
  for (const corr of sorted) {
    ...
    segments.push({
      text: content.slice(idx, idx + search.length),
      correction: corr,
    });
...
const fetchAuditData = async () => {
  ...
  try {
    ...
    error.value =
      error_ instanceof Error ? error_.message : t("failed_to_load_audit");
  } finally {
    loading.value = false;
  }
};
...
const highlightCorrections = (
  ...
  for (const correction of corrections) {
    const text =
      type === "original" ? correction.originalText : correction.correctedText;
    ...
    highlightedContent = highlightedContent.replace(
      regex,
      `<span class="text-${color} font-weight-bold">$1</span>`,
    );
  }
...
const saveNotes = async () => {
  ...
  try {
    ...
    console.error("Failed to save notes:", error_);
...
const handleVersionRequest = async (data: {
  feedback: string;
  newContext?: string;
}) => {
  if (!audit.value) return;
  ...
  try {
    ...
    console.error("Failed to submit version request:", error_);
